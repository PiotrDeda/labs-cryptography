...
}.ForEach(c => Console.Write(Decode(ElGamalDecrypt(c, p, a))));

//////////////////////////////

static long ElGamalDecrypt((long y1, long y2) cipher, long p, ulong a) =>
    cipher.y2 * ModInverse(ModPow(cipher.y1, a, p), p) % p;

static string Decode(long value) =>
    $"{(char)(value / 676 + 'A')}{(char)(value / 26 % 26 + 'A')}{(char)(value % 26 + 'A')}";
...
static long ModInverse(long a, long n) => (ExtGCD(a, n).x + n) % n;
